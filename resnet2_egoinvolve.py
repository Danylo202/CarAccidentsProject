...
class RawVideoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return video, label_acc, label_ego
...
class PretrainedMTLModel(nn.Module):
    def __init__(self, hidden_size=256):
        super(PretrainedMTLModel, self).__init__()
        
        # 1. Завантажуємо переднавчену ResNet18 
        ...
        # 2. LSTM: на вхід іде 512 фіч від ResNet18
        self.lstm = nn.LSTM(input_size=512, hidden_size=hidden_size, 
                            num_layers=2, batch_first=True, dropout=0.3)
        
        # 3. Дві незалежні голови замість одного класифікатора
        self.head_accident = nn.Linear(hidden_size, 1)
        self.head_ego = nn.Linear(hidden_size, 1)
    ...
    def forward(self, x):
        ...
        # Пропускаємо вихід LSTM через обидві голови
...
if __name__ == '__main__':
    ...
    model = PretrainedMTLModel().to(device)
    ...
    # print(f"Навчання Pretrained MTL-моделі розпочато на пристрої: {device}")
    ...
    final_model = PretrainedMTLModel().to(device)
    ...
    with open(results_file, 'a', encoding="utf-8") as f:
        f.write("\n" + "="*50 + "\n")
        f.write(f"MODEL: PretrainedMTLModel (MTL: Accident + Ego)\n")

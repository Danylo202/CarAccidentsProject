...
На вхід ідуть 50 кадрів, кожен має розміри 224x224 і 3 канали (RGB). Тож, здавалось би, input size має форму (50, 3, 224, 224). Проте було ухвалене рішення брати не лише поточний кадр, а й різницю (матричну) між ним і попереднім, щоб модель краще вчилась бачити рух. Тому кожному відео відповідає тензор форми (50, 6, 224, 224).
Щодо архітектури самої моделі, її можна розділити на дві основні частини: витягання ознак, і LSTM.
Перша частина (feature extraction) фактично включає 5 шарів, фактично в кожному відбувається стискання картини в розмірах (вдвічі), проте збільшення кількості перспектив (каналів) через Residual Network архітектуру, кожен з яких накопичує певні ознаки. Тож в результаті кожен кадр (3, 224, 224) перетворюється на вектор з 256 ознак.
Далі ці 50 векторів подаються в LSTM-шар, який вчиться прогнозувати імовірність аварії в певний момент, виходячи з даних про попередні кадри.
Більшість імовірностей - нулі (навіть на відео з аваріями одиниці - лише останні кадри, коли аварія сталась), тож щоб модель краще реагувала на аварії, для її навчання береться BCEWithLogitsLoss зі штрафом за False negative (pos_weight = [3.0]). 
Для оптимізації learning rate під час навчання використовується scheduler.
Окрім loss, важливими є такі статистичні метрики, як recall і precision, а саме баланс між ними.
Також була спроба використати focal loss, що не призвело до покращення recall і precision.
